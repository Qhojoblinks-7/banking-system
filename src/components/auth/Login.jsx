...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/user-account-overview"), 2000);
    } catch (error) {
      console.error("Login error:", error);
      const errorMessage = error?.message || "Invalid email or password.";
      showAlert(`Login failed: ${errorMessage}`, "bg-red-100 text-red-700");
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-sky-50 font-sans p-4 relative">
      ...
      <div className="w-full max-w-md mx-auto shadow-lg bg-white p-6 rounded-lg">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="flex space-x-2">
            <button
              type="submit"
              className="w-full bg-green-700 text-white font-semibold rounded px-4 py-2 hover:bg-green-800 disabled:bg-gray-400"
              aria-label="Login"
              disabled={processing}
            >
